...
export default function Page() {
  ...
  return (
    <div className="space-y-4">
      <SellerPageHeader title="Toutes les commandes" description="Suivi de toutes vos commandes." action={<>
        <SellerHeaderActions>
          <SellerActionButton variant="secondary" onClick={() => notifySoon('Filtrer')}>Filtrer</SellerActionButton>
          <SellerActionButton variant="outline" onClick={() => notifySoon('Exporter')}>Exporter</SellerActionButton>
        </SellerHeaderActions></>} />
      <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="N° commande, client..." className="w-full md:max-w-md px-4 py-2.5 rounded-lg border border-gray-200 text-sm outline-none focus:ring-2 focus:ring-brand-orange" />
      <SellerCard>
        {filtered.length === 0 ? (
            ...
            {filtered.map((o) => (
              <div key={o.id} className="border border-gray-100 rounded-xl p-4 flex flex-col xl:flex-row xl:items-center justify-between gap-3">
                ...
                <RowActions>
                      ...
                      <SellerActionButton size="sm" variant="secondary" onClick={() => notifySoon("Préparer")}>Préparer</SellerActionButton>
